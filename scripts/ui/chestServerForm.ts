...
const chestLabel = label("chest_label", "#title_text")
  .offset(7, 10)
  .anchor("top_left")
  .size("90%", "default")
  .color("$title_text_color")
...
const nonRendererItem = boundImage("non_renderer_item")
  .size(16, 16)
  .bindings(...nonRendererItemBindings());

const itemAmount = label("item_amount", "#stack_size")
  .offset(0, 1)
  .shadow()
  .textAlignment("left")
  .anchor("bottom_right")
...
const inventoryButtonAmount = panel("inventory_button_amount")
  .rawProp("offset", "$offset")
  .controls(itemAmount);

const defaultControl = panel("default_control")
  .wrapChildren()
  .layer(3)
  .controls(
...
const itemDetails = custom("item_details")
  .renderer("hover_text_renderer")
  .allowClipping(false)
  .layer(30)
  .bindings(...hoverTextBindings(58));

const highlightSlot = image("highlight_slot", "textures/ui/highlight_slot")
  .size(18, 18)
  .layer(0)
  .bindings(...tPrefixVisible);

const focusBorder = image("focus_border", "textures/ui/focus_border_white")
  .size(18, 18)
  .layer(1)
  .bindings(...tPrefixVisible);

const hoveringImage = panel("hovering_image")
  .size(18, 18)
  .controls(
    itemDetails,
    extendRaw("item_block", "beacon.item_renderer", {
...
const hoverControl = panel("hover_control")
  .wrapChildren()
  .rawProp("offset", "$offset")
  .controls(hoveringImage);

const pressedControl = panel("pressed_control")
  .wrapChildren()
  .controls(
    extendRaw("item_block", "beacon.item_renderer", {
      size: [16, 16],
...
const badgeImage = boundImage("badge_image")
  .size(8, 8)
  .offset(1, 1)
  .anchor("top_left")
  .layer(5)
...
const slotBadge = panel("slot_badge")
  .rawProp("offset", "$offset")
  .controls(badgeImage);

const chestItem = stackPanel("chest_item", "vertical")
...
const inventoryChestGridImage = chestGridImage("inventory_chest_grid_image", {
...
const singleChestGridImage = chestGridImage("single_chest_grid_image", {
...
const tinyChestGridImage = chestGridImage("tiny_chest_grid_image", {
...
const smallChestGridImage = chestGridImage("small_chest_grid_image", {
...
const largeChestGridImage = chestGridImage("large_chest_grid_image", {
...
const questsGridImage = chestGridImage("quests_grid_image", {
...
const questsGridLargeImage = chestGridImage("quests_grid_large_image", {
...
const pokebuilderGridImage = chestGridImage("pokebuilder_grid_image", {
...
const pokebuilderGridLargeImage = chestGridImage(
...
const backpackGrid = chestGridImage("backpack_grid", {
...
const auctionHouseGrid = chestGridImage("auction_house_grid", {
...
const chestPanel = panel("chest_panel")
  .wrapChildren()
  .controls(
    ...
    ref("pokebuilder_grid_large_image@chest_ui.pokebuilder_grid_large_image"),
...
const ui = defineUI("chest_ui", (ns) => {
  ...
  defaultControl.addToNamespace(ns);
  hoverControl.addToNamespace(ns);
  pressedControl.addToNamespace(ns);
  const registeredInventoryButton = inventoryButton.addToNamespace(ns);
  extend("ui_chest_item", registeredInventoryButton)
    .variable("offset", [-1, -1])
    .bindings(...formButtonPrefixVisibility("cht:"))
    .addToNamespace(ns);
  extend("ui_inventory_item", registeredInventoryButton)
    .variable("offset", [-1, 10])
    .bindings(...formButtonPrefixVisibility("inv:"))
    .addToNamespace(ns);
  extend("ui_hot_bar_item", registeredInventoryButton)
    .variable("offset", [-1, 13])
    .bindings(...formButtonPrefixVisibility("hot:"))
    .addToNamespace(ns);
  slotBadge.addToNamespace(ns);
  chestItem.addToNamespace(ns);
  inventoryChestGridImage.addToNamespace(ns);
  singleChestGridImage.addToNamespace(ns);
  tinyChestGridImage.addToNamespace(ns);
  smallChestGridImage.addToNamespace(ns);
  largeChestGridImage.addToNamespace(ns);
  questsGridImage.addToNamespace(ns);
  questsGridLargeImage.addToNamespace(ns);
  pokebuilderGridImage.addToNamespace(ns);
